...
function Avatar({ initials, size = "lg" }: { initials: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("rounded-full bg-neutral-800 flex items-center justify-center font-black text-white", sz)}>
      {initials}
    </div>
  );
}
...
function SectionCard({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={cn("rounded-2xl bg-neutral-900 border border-neutral-800 p-6", className)}>
      <h3 className="text-sm font-bold tracking-widest text-neutral-400 uppercase mb-5">{title}</h3>
      {children}
    </div>
  );
}
...
function StatPill({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className={cn("flex flex-col gap-1 rounded-xl px-4 py-3 text-white", color)}>
...
function QuickLink({ icon, label, href }: { icon: React.ReactNode; label: string; href: string }) {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.97 }}
      className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-neutral-800 hover:bg-neutral-700 transition-colors cursor-pointer"
    >
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <header className="sticky top-0 z-30 bg-black/80 backdrop-blur border-b border-neutral-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-black text-white">{mockEmployee.name}</p>
            <p className="text-xs text-neutral-500 font-medium">{mockEmployee.role}</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setShowNotifs(!showNotifs)} className="relative p-2 rounded-full bg-neutral-900 hover:bg-neutral-800 transition-colors">
            <Bell size={18} />
      ...
      >
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          {/* Profile card */}
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-6 flex items-center gap-5">
            <Avatar initials={mockEmployee.initials} size="lg" />
            <div className="min-w-0">
              <p className="text-lg font-black text-white truncate">{mockEmployee.name}</p>
              <p className="text-sm text-neutral-400 font-medium truncate">{mockEmployee.role}</p>
              ...
              <a href="/profile" className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-white hover:text-neutral-300 transition-colors">
          ...
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-6 space-y-4">
            <h3 className="text-xs font-bold tracking-widest text-neutral-400 uppercase">Today's Attendance</h3>
            <div className={cn(
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="text-white font-black">{checkedIn ? checkInTime : "--:--"}</p>
              </div>
              <div>
                <p className="text-neutral-500 font-medium">Check Out</p>
                <p className="text-white font-black">{!checkedIn && checkInTime !== mockAttendance.checkIn ? new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) : "--:--"}</p>
              </div>
            </div>
          </div>
          ...
          {/* Today's hours */}
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-6 space-y-4">
            <h3 className="text-xs font-bold tracking-widest text-neutral-400 uppercase">Today's Working Hours</h3>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="space-y-1">
              {mockAttendance.weekData.filter(d => d.status !== "weekend").map((d, i) => (
                <div key={i} className="flex items-center gap-3">
                  <span className="text-xs w-8 text-neutral-500 font-bold">{d.day}</span>
                  ...
                  <span className="text-xs text-neutral-400 w-10 text-right font-medium">{d.hours}h</span>
                </div>
              ))}
            </div>
          </SectionCard>
          ...
          <SectionCard title="Leave Balance">
            <div className="space-y-5">
                ...
                <div key={label}>
                  <div className="flex justify-between text-sm font-medium mb-2">
                    ...
                    <span className="text-white font-black">
                      {data.used} <span className="text-neutral-500 font-medium">/ {data.total} used</span>
                    </span>
                  </div>
                  <div className="h-2 rounded-full bg-neutral-800 overflow-hidden">
                    <motion.div
              ...
              <a href="/leave" className="mt-2 inline-flex items-center gap-1.5 text-sm font-black text-white hover:text-neutral-300 transition-colors">
                ...
                <tbody className="divide-y divide-neutral-800/50">
                  {mockLeaveRequests.map(req => (
                    <tr key={req.id} className="hover:bg-neutral-800/30 transition-colors">
                      <td className="py-4 pr-6 text-neutral-400 font-mono text-xs">{req.id}</td>
                      <td className="py-4 pr-6 font-bold text-white whitespace-nowrap">{req.type}</td>
                      <td className="py-4 pr-6 text-neutral-400 whitespace-nowrap">{req.from} – {req.to}</td>
                      <td className="py-4 pr-6 text-white font-black">{req.days}d</td>
                      <td className="py-4 pr-6 text-neutral-400 max-w-xs truncate">{req.reason}</td>
                      <td className="py-4">
            ...
            <div className="space-y-4">
              {mockRecentActivity.map(act => (
                <div key={act.id} className="flex items-start gap-3">
                  <div className="mt-0.5 p-2 rounded-xl bg-neutral-800 text-neutral-400">
